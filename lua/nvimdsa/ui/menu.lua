...
---@class nvimdsa.ui.Menu : lc.ui.Renderer
---@field cursor { idx: integer, prev: integer[]? }
local Menu = Renderer:extend("NvimDSAMenu")
...
function Menu:autocmds()
    local group_id = api.nvim_create_augroup("nvimdsa_menu", { clear = true })
...
---Load and display a named page from lua/nvimdsa/ui/pages/<name>.lua
---@param name string
function Menu:set_page(name)
    self:cursor_reset()

    local ok, page = pcall(require, "nvimdsa.ui.pages." .. name)
    if ok then
        self:replace({ page })
    else
        log.error(page)
...
function Menu:init()
    ...
    _NVIMDSA_state.menu = self
end

---@type fun(): nvimdsa.ui.Menu
local NvimDSAMenu = Menu
